...
<script>
import { getResetPaCode, identifyResetPaCode } from '@/api/user'
  ...
  methods: {
    ...
    async handleClickTimer() {
      try {
        ...
        const res = await getResetPaCode(formData)
        // 定时器倒计时
        this.codeTimer = window.setInterval(() => {
          this.codeTimeSeconds--;
          if (this.codeTimeSeconds <= 0) {
            window.clearInterval(this.codeTimer);
            this.codeTimeSeconds = initCodeTimeSeconds;  // 定时器回到初始状态
            this.codeTimer = null;  // 回到初始重新为空
          }
        }, 1000);
      } catch (error) {
        console.log(error)
    ...
    async handleClick() {
      try {
        ...
        if (!phone) {
          ...
          const res = await identifyResetPaCode(formData)
          if (res.data.status) {
