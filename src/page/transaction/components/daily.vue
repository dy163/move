<template>
  <div>
    <div id="myChart" ref="daily" class="text"></div>
  </div>
</template>

<script>
import echarts from "echarts";
import { klineDay } from "@/api/stocktime";
  ...
  methods: {
    async handlekineDaily(q) {
      try {
         ...
         * 加载K线图
         */
        const daily = this.$refs.daily;
        const myChart = echarts.init(daily)    // 这个位置不能使用ref属性取值

        // const myChart = echarts.init(document.getElementById("myChart"));
        var upColor = "#ec0000";
        var downColor = "#00da3c";
        // 数据意义：开盘(open)，收盘(close)，最低(lowest)，最高(highest)
        var data0 = splitData(this.dailydata);
