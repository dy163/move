...
export default {
  ...
  methods: {
    ...
    // 自定义时间加载
    async handleConfirm() {
      const starttime = window.localStorage.getItem('inquireStarttime')
      const endtime = window.localStorage.getItem('inquireEndtime')
      try {
        if (!starttime && !endtime) {
          ...
          if (res.data.result === null) {
            ...
            if (this.listNum.length < this.pageSize) {
              this.upFinished = true;
              window.localStorage.removeItem("inquireStarttime");
              window.localStorage.removeItem("inquireEndtime");
            }
          }
        }
      } catch (error) {
